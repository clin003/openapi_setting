...
func Save() error {
	if v, err := appkey.Get(); err != nil {
		...
		app_conf.AppConf.Sdk.Taobao.Appkey = v
	}

	if v, err := appsecret.Get(); err != nil {
		return err
	} else {
		app_conf.AppConf.Sdk.Taobao.Appsecret = v
	}

	if v, err := adzoneid.Get(); err != nil {
		return err
	} else {
		app_conf.AppConf.Sdk.Taobao.Adzoneid = v
	}

	if v, err := pid.Get(); err != nil {
		return err
	} else {
		app_conf.AppConf.Sdk.Taobao.Pid = v
		app_conf.AppConf.App.Tb.Pid = v
	}
	return nil
}
...
func Show(parent fyne.Window) fyne.CanvasObject {
	...
	inputPid.SetPlaceHolder("请输入pid: 例子 mm_1_2_3")
	...
	content := container.NewVBox(head, contentAppkey, contentAppseret, contentAdzoneid, contentPid)
	return content
}
